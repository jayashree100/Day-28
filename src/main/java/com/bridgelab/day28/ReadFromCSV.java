...
public class ReadFromCSV {
	public static String writeData1(Map<String, AddressBook> addressBookMap) {
		...
		File file = new File("D:Contact.csv");
	...
	public static boolean ReadDataCSV() {
		String fileName = "D:Contact.csv";
		try (Stream<String> lines = Files.lines(Paths.get(fileName))) {
			List<Object> values = lines.map(line -> Arrays.asList(line.split(","))).collect(Collectors.toList());
